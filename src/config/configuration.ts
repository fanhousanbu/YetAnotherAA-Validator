export default () => {
  ...
  console.log(
    `   - Validator Contract: ${process.env.VALIDATOR_CONTRACT_ADDRESS}`,
  );
  ...
  return {
    ...
    // Gossip Network
    gossipPublicUrl:
      process.env.GOSSIP_PUBLIC_URL || `ws://localhost:${port}/ws`,
    gossipBootstrapPeers: parseBootstrapPeers(
      process.env.GOSSIP_BOOTSTRAP_PEERS || "",
    ),
    ...
    gossipHeartbeatInterval: parseInt(
      process.env.GOSSIP_HEARTBEAT_INTERVAL || "10000",
      10,
    ),
    gossipSuspicionTimeout: parseInt(
      process.env.GOSSIP_SUSPICION_TIMEOUT || "30000",
      10,
    ),
    gossipCleanupTimeout: parseInt(
      process.env.GOSSIP_CLEANUP_TIMEOUT || "60000",
      10,
    ),
    gossipMaxMessageHistory: parseInt(
      process.env.GOSSIP_MAX_MESSAGE_HISTORY || "1000",
      10,
    ),
  };
};
